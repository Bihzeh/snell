...
- **Config:** per-module JSON on disk (`.minecraft/config/snell/`), schema-versioned, via kotlinx.serialization.
...
1. **Identity proof:** client proves control of its MC UUID to the Snell backend (challenge validated against the Mojang session — exact mechanism is an ADR detail), receiving a Snell session token.
...
3. **Render:** player-renderer mixin draws equipped cosmetics (Blaze3D). Result: Snell users see each other's cosmetics; non-users see vanilla; the game server is uninvolved.
...
snell/
  ...
  mod/
    ...
    src/main/resources/fabric.mod.json, snell.mixins.json
  launcher/
...
**Phase 3 — Backend & cosmetics.** Ktor + Postgres; Snell identity keyed to MC UUID; cosmetics catalog, ownership/entitlements; **in-game cosmetic sync (backend-mediated)** with the player-renderer mixin live.
