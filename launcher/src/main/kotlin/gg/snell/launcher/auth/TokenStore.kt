 ...
 * user's machine and are NEVER sent to the Snell backend (ADR-0006).
 */
...
class FileTokenStore(private val dir: Path) : TokenStore {
    ...
    override fun loadRefreshToken(account: String): String? {
        ...
        return try {
            ...
            System.err.println("Snell: WARNING — stored token failed authentication (corrupt or tampered); discarding.")
    ...
    private fun restrict(path: Path) {
        runCatching { Files.setPosixFilePermissions(path, PosixFilePermissions.fromString("rw-------")) }
            .onFailure {
                System.err.println(
                    "Snell: WARNING — could not restrict permissions on ${path.fileName} " +
