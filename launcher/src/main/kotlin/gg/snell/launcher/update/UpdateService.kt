...
class UpdateService(
    ...
    private val fetchReleases: suspend () -> String = {
        Net().use { it.text(RELEASES_URL, mapOf("User-Agent" to "Snell-Launcher", "Accept" to "application/vnd.github+json")) }
    },
) {
    ...
    suspend fun apply(info: UpdateInfo, onStatus: (String) -> Unit) {
        ...
        val dir = Files.createTempDirectory("snell-update")
