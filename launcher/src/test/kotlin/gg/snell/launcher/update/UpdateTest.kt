...
class UpdateServiceTest {
    ...
    @Test fun `picks newest release with an installer, newer than current`() {
        val releases = listOf(
            release("v0.1.4", "Snell-0.1.4.exe", "Snell-0.1.4.msi"),
            release("v0.1.3", "Snell-0.1.3.exe"),
        )
    ...
    @Test fun `returns null when already up to date`() {
        val releases = listOf(release("v0.1.3", "Snell-0.1.3.exe"))
    ...
    @Test fun `extracts sha256 for the right file`() {
        val sums = "abc  other.exe\n" + "b".repeat(64) + "  Snell-0.1.4.exe\n"
        assertEquals("b".repeat(64), UpdateService.sha256For(sums, "Snell-0.1.4.exe"))
    }
}
