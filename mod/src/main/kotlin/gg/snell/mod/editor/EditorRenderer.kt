...
import gg.snell.mod.render.HudModuleRender
 ...
 * states. POSITION: live preview + drag outlines + the SNELL wordmark + a "Mods" button. GRID: a
...
class EditorRenderer {
    ...
    private fun drawPosition(
        ...
        for (b in boxes) {
            ...
            val color = when {
                b.id == state.selectedId -> SnellPalette.gold
                b.id == hover -> lighten(SnellPalette.primary, 0.25f)
                !enabled -> SnellPalette.error
                else -> SnellPalette.outline
            }
            canvas.border(b.rect.left - 2, b.rect.top - 2, b.rect.width + 4, b.rect.height + 4, color)
        }
        for (gx in state.activeGuidesX) canvas.fill(gx, 0, 1, screenH, SnellPalette.gold)       // alignment guides
        for (gy in state.activeGuidesY) canvas.fill(0, gy, screenW, 1, SnellPalette.gold)
        state.selectedId?.let { sid ->                                                            // resize grip
            boxes.firstOrNull { it.id == sid }?.rect?.let { b ->
                val h = PositionLayout.resizeHandle(b)
                canvas.fill(h.left, h.top, h.width, h.height, SnellPalette.gold)
                canvas.border(h.left, h.top, h.width, h.height, white)
            }
        }
        val hint = "Drag to reposition  ·  Mods to customize  ·  Esc / Done to save"
        canvas.drawText((screenW - canvas.textWidth(hint)) / 2, screenH - 16, hint, SnellPalette.text2) // bottom-center, clear of corner HUD
    ...
    private fun drawCard(canvas: EditorCanvas, r: Rect, name: String, enabled: Boolean, hover: Boolean) {
        val borderC = if (hover) lighten(SnellPalette.primary, 0.25f) else SnellPalette.outline
        card(canvas, r, SnellPalette.elevated, borderC)
        canvas.fill(r.left + 1, r.top + 1, 3, r.height - 2, if (enabled) SnellPalette.primary else SnellPalette.outline) // accent strip
        val innerL = r.left + 11
        canvas.drawText(innerL, r.top + 9, ellipsize(canvas, name, r.width - 22), if (enabled) white else SnellPalette.text2)
        val ts = GridLayout.toggleSwitch(r) // shared hit-area: switch widget + its label
        switch(canvas, Rect(ts.left, ts.top, 26, 13), enabled)
        canvas.drawText(ts.left + 32, ts.top + 3, if (enabled) "On" else "Off", SnellPalette.text2)
        canvas.drawText(r.right - 12, r.top + (r.height - canvas.lineHeight) / 2, ">", SnellPalette.text2) // opens affordance
    }
    ...
    private fun drawSelectedOutline(canvas: EditorCanvas, boxes: List<ElementBox>, state: EditorState) {
        ...
        canvas.border(b.left - 2, b.top - 2, b.width + 4, b.height + 4, SnellPalette.gold)
    }
    ...
    private fun drawCustomize(canvas: EditorCanvas, screenW: Int, screenH: Int, mouseX: Int, mouseY: Int, modules: ModuleManager, state: EditorState) {
        ...
        canvas.gradientV(popup.left + 1, popup.top + 1, popup.width - 2, hh - 2, lighten(SnellPalette.elevated, 0.12f), SnellPalette.elevated)
        canvas.fill(popup.left + 1, popup.top + hh - 1, popup.width - 2, 1, SnellPalette.outline)
        canvas.fill(popup.left + CustomizeLayout.PAD, popup.top + (hh - 7) / 2, 4, 7, SnellPalette.gold) // accent tab
        canvas.drawText(popup.left + CustomizeLayout.PAD + 9, popup.top + (hh - canvas.lineHeight) / 2, ellipsize(canvas, module.displayName, popup.width - 50), SnellPalette.text)
        val cb = CustomizeLayout.closeButton(popup)
        button(canvas, cb, "x", primary = false, hover = cb.contains(mouseX, mouseY)); round(canvas, cb, SnellPalette.elevated)

        if (hud != null) {
            drawStyleControls(canvas, hud, popup, state, mouseX, mouseY)
        } else {
            val en = CustomizeLayout.enableToggle(popup)
            canvas.drawText(en.left, en.top + 3, if (module.enabled) "Enabled" else "Disabled", if (module.enabled) white else SnellPalette.text2)
    ...
    private fun drawStyleControls(canvas: EditorCanvas, module: HudModule, popup: Rect, state: EditorState, mouseX: Int, mouseY: Int) {
        ...
        c["preview"]?.rect?.let { r ->
            ...
            canvas.border(r.left, r.top, r.width, r.height, SnellPalette.outline)
        }
        ...
        c["hex"]?.rect?.let { r ->
            canvas.fill(r.left, r.top, r.width, r.height, darken(SnellPalette.elevated, 0.12f))
            canvas.border(r.left, r.top, r.width, r.height, if (state.isHexFocused) SnellPalette.gold else SnellPalette.outline)
            round(canvas, r, SnellPalette.surface)
            val text = if (state.isHexFocused) "#" + state.hexText + "_" else HexColor.encode(active)
            canvas.drawText(r.left + 5, r.top + (r.height - canvas.lineHeight) / 2 + 1, text, SnellPalette.text)
        }
        ...
        targets.forEachIndexed { i, t ->
            ...
            val bg = when { on -> blend(SnellPalette.elevated, SnellPalette.gold, 0.16f); hov -> lighten(SnellPalette.elevated, 0.12f); else -> SnellPalette.elevated }
            canvas.fill(r.left, r.top, r.width, r.height, bg)
            canvas.border(r.left, r.top, r.width, r.height, if (on) SnellPalette.gold else SnellPalette.outline)
            round(canvas, r, SnellPalette.surface)
            canvas.drawText(r.left + 7, r.top + (r.height - canvas.lineHeight) / 2 + 1, t.label, if (on) white else SnellPalette.text2)
            ...
            canvas.border(sx, sy, sw, sw, SnellPalette.outline)
        }
        ...
        if (oc > 0) {
            ...
            module.toggles.forEachIndexed { i, t ->
                ...
                canvas.drawText(r.left + 6, r.top + (r.height - canvas.lineHeight) / 2 + 1, t.label, if (on) white else SnellPalette.text2)
        ...
        for (id in TOGGLE_ROW) {
            ...
            canvas.drawText(r.left + 6, r.top + (r.height - canvas.lineHeight) / 2 + 1, label(id), if (on) white else SnellPalette.text2)
            switch(canvas, Rect(r.right - 26, r.top + 2, 24, r.height - 4), on)
        }
        val sm = c["scale-"]?.rect; val sp = c["scale+"]?.rect
        if (sm != null && sp != null) {
            button(canvas, sm, "-", hover = sm.contains(mouseX, mouseY)); round(canvas, sm, SnellPalette.surface)
            button(canvas, sp, "+", hover = sp.contains(mouseX, mouseY)); round(canvas, sp, SnellPalette.surface)
            val v = "x%.2f".format(st.scale)
            canvas.drawText((sm.right + sp.left) / 2 - canvas.textWidth(v) / 2, sm.top + (sm.height - canvas.lineHeight) / 2 + 1, v, SnellPalette.text2)
        }
        c["reset"]?.rect?.let { button(canvas, it, "Reset", hover = it.contains(mouseX, mouseY)); round(canvas, it, SnellPalette.surface) }

        CustomizeLayout.SWATCHES.forEachIndexed { i, col ->
            c["swatch:$i"]?.rect?.let { r ->
                canvas.fill(r.left, r.top, r.width, r.height, black or SnellColor.rgbOf(col))
                val selected = SnellColor.rgbOf(active) == SnellColor.rgbOf(col)
                if (selected) canvas.border(r.left - 1, r.top - 1, r.width + 2, r.height + 2, SnellPalette.gold)
                else canvas.border(r.left, r.top, r.width, r.height, SnellPalette.outline)
            }
        }
    }

    private fun sectionLabel(canvas: EditorCanvas, cap: Rect, text: String) {
        canvas.drawText(cap.left + 1, cap.top, text, SnellPalette.text2)
        canvas.fill(cap.left, cap.bottom - 2, cap.width, 1, SnellPalette.outline)
    }

    private fun rowHover(canvas: EditorCanvas, r: Rect, mouseX: Int, mouseY: Int) {
        if (r.contains(mouseX, mouseY)) canvas.fill(r.left, r.top, r.width, r.height, lighten(SnellPalette.surface, 0.07f))
    }
    ...
    private fun drawSvSquare(canvas: EditorCanvas, r: Rect, state: EditorState) {
        ...
        for (x in 0 until r.width) {
            ...
            val top = black or SnellColor.hsvToRgb(state.colorH, s, 1f)
    ...
    private fun drawHueBar(canvas: EditorCanvas, r: Rect, state: EditorState) {
        for (i in 0 until 6) {
            ...
            val top = black or SnellColor.hsvToRgb(i * 60f, 1f, 1f)
            val bottom = black or SnellColor.hsvToRgb((i + 1) * 60f, 1f, 1f)
    ...
    private fun drawAlphaBar(canvas: EditorCanvas, r: Rect, state: EditorState) {
        checker(canvas, r.left, r.top, r.width, r.height)
        val rgb = SnellColor.hsvToRgb(state.colorH, state.colorS, state.colorV)
    ...
    private fun panel(canvas: EditorCanvas, r: Rect) {
        ...
        canvas.gradientV(r.left, r.top, r.width, r.height, lighten(SnellPalette.surface, 0.05f), SnellPalette.surface)
        canvas.border(r.left, r.top, r.width, r.height, SnellPalette.outline)
    }

    private fun header(canvas: EditorCanvas, panel: Rect, headerH: Int, title: String) {
        canvas.gradientV(panel.left + 1, panel.top + 1, panel.width - 2, headerH - 1, lighten(SnellPalette.elevated, 0.10f), SnellPalette.elevated)
        canvas.fill(panel.left + 1, panel.top + headerH, panel.width - 2, 1, SnellPalette.outline)
        val tw = canvas.textWidth(title)
        canvas.drawText(panel.left + (panel.width - tw) / 2, panel.top + (headerH - canvas.lineHeight) / 2, title, SnellPalette.gold)
    }
    ...
    private fun button(canvas: EditorCanvas, r: Rect, text: String, primary: Boolean = false, hover: Boolean = false) {
        val base0 = if (primary) SnellPalette.primary else SnellPalette.elevated
        ...
        canvas.border(r.left, r.top, r.width, r.height, if (primary) lighten(SnellPalette.primary, 0.15f) else SnellPalette.outline)
    ...
    private fun switch(canvas: EditorCanvas, r: Rect, on: Boolean) {
        val track = if (on) SnellPalette.primary else darken(SnellPalette.elevated, 0.18f)
        canvas.gradientV(r.left, r.top, r.width, r.height, lighten(track, 0.18f), track)
        canvas.border(r.left, r.top, r.width, r.height, if (on) lighten(SnellPalette.primary, 0.2f) else SnellPalette.outline)
        val k = r.height - 4
